...
# -*- coding: utf-8 -*-
"""
Kepler KOI ML Pipeline (Hackathon-ready) - v2 robust I/O
--------------------------------------------------------
- Adds robust table reader with delimiter auto-detection and fallbacks
- Optional --sep and --on_bad_lines
- Everything else identical to v1

Usage:
    python kepler_ml_pipeline_v2.py --csv path/to/kepler.csv [--binary] [--positive CONFIRMED] [--smote] [--sep "\t"]
"""
import argparse
import os
import sys
...
TARGET_DEFAULT = "koi_pdisposition"  # 'CANDIDATE', 'FALSE POSITIVE'
...
def read_table(path: str, sep: Optional[str], on_bad_lines: str = "error") -> pd.DataFrame:
    """
    Robust reader:
    1) If sep is provided, use it directly.
    2) Try pandas default (comma).
    3) Try python engine with sep=None (auto-sniff).
    4) Try common alternatives: tab, semicolon, pipe.
    5) As last resort, try on_bad_lines='skip' if not already set.
    """
    tried = []
...
def load_and_prepare(
    ...
    # Quick sanity check to detect the very common mistake of passing the script file instead of a CSV
    if df.shape[1] == 1 and df.columns.tolist() == [df.columns[0]] and df.columns[0].endswith(".py"):
        print("[ERROR] It looks like you passed a Python script instead of a CSV/TSV with KOI data.", file=sys.stderr)
        raise SystemExit(2)
    ...
    # Keep only selected features + target (if target exists)
    if target_col not in df.columns:
        raise KeyError(f"Target column '{target_col}' not found in file. Available columns: {list(df.columns)[:30]} ...")
...
def parse_args():
    p = argparse.ArgumentParser(description="Kepler KOI ML Pipeline (v2 robust I/O)")
    ...
    p.add_argument("--sep", default=None, help="Field separator override, e.g., '\\t' for TSV, ';' for semicolon")
    p.add_argument("--on_bad_lines", default="error", choices=["error", "warn", "skip"],
                   help="How to handle bad lines (default: error)")
    return p.parse_args()
