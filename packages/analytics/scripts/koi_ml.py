...
def train_from_harmonized(
    ...
    rename_map = {
        ...
        "koi_srad": "stellar_radius_solar",  # Included now
    ...
    # We remove engineered features as they were not generated in generate_dataset_keppler.py
    # We remove koi_impact and koi_sma as they are not in KOI_All_Filtrated.csv
    # We include koi_srad (stellar_radius_solar)
    keep_cols = [
        "orbital_period_days",
    ...
    df = ensure_labels(df)
    df.to_csv(outdir / "exoplanets_used.csv", index=False)
    ...
    # Verificar que koi_score efectivamente fue excluido
    all_features = num_cols + cat_cols
    if 'koi_score' in all_features:
        print("⚠️  WARNING: koi_score encontrado en features (no debería estar)")
    else:
        print(f"✓ koi_score correctamente excluido. Features usadas: {len(all_features)}")
    ...
    metrics_ext = {
        ...
        "koi_score_excluded": True,
        "model_notes": "koi_score excluido por decisión basada en experimento (solo +0.32% mejora)"
    }
    ...
    cfg = {
        ...
        "feature_importances": [
             ...
             for feat, imp in zip(num_cols + cat_cols, pipe.named_steps["rf"].feature_importances_)
        ],
        "test_size": test_size,
        "auto_drop_ids": auto_drop_ids,
        "exclude_cols": exclude_cols,
        "koi_score_excluded": True,
        "model_version": "v1_without_koi_score_clean",
    }
...
def parse_args():
    ap = argparse.ArgumentParser(
        description="Entrenar RandomForest desde el dataset KOI Filtrado (SIN koi_score)."
    )
    # Default to the KOI_All_Filtrated.csv now
    ...
    # Default output path: apps/server/models (relative to this script)
    default_out = Path(__file__).resolve().parent.parent.parent.parent / "apps/server/models"
    
    ap.add_argument(
        "--out", type=str, default=str(default_out), help="Carpeta de salida."
    )
    ...
    ap.add_argument(
        ...
        type=str,
        default="",
        help="Lista de columnas a excluir adicionales, separadas por coma.",
    )
    ap.add_argument(
        "--no-auto-drop-ids",
        action="store_true",
