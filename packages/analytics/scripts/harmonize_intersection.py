...
def unify_disposition(val, source=None):
    ...
    # TOI special cases
    if source == "TOI":
        if s in ("CP", "KP", "CONFIRMED"):
            return "CONFIRMED"
        if s in ("PC", "APC", "CANDIDATE"):
            return "CANDIDATE"
        if s in ("FP", "FA", "FALSE POSITIVE", "FALSE_POSITIVE"):
            return "FALSE_POSITIVE"
    # generic terms
    if "CONFIR" in s or s == "CP" or s == "KP":
        return "CONFIRMED"
    if "CAND" in s or s == "PC" or s == "APC":
        return "CANDIDATE"
    if "FALSE" in s or "REFUT" in s or s in ("FP", "FA"):
        return "FALSE_POSITIVE"
    # if it's already one of the target labels
    if s in ("CONFIRMED", "CANDIDATE", "FALSE_POSITIVE"):
        return s
    return np.nan
...
if "final_disposition" in koi_p.columns:
    koi_p["final_disposition"] = koi_p["final_disposition"].apply(
        lambda v: unify_disposition(v, source="KOI")
    )
if "final_disposition" in k2_p.columns:
    k2_p["final_disposition"] = k2_p["final_disposition"].apply(
        lambda v: unify_disposition(v, source="K2")
    )
if "final_disposition" in toi_p.columns:
    toi_p["final_disposition"] = toi_p["final_disposition"].apply(
        lambda v: unify_disposition(v, source="TOI")
    )
...
def filter_essential(df, mission):
    ...
    print(
        f"[{mission}] filter essential: {before} -> {after} rows (dropped {before - after})"
    )
    return df_clean
