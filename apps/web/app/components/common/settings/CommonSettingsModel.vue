...
const Schema = computed(() =>
    z.object({
        orbital_period_days: z.coerce
            ...
            .min(0.0, { message: t("pages.predict.form.validation.orbital_period_range") })
            .max(200000.0, { message: t("pages.predict.form.validation.orbital_period_range") })
            .refine(
                (val) => !isNaN(val) && val > 0,
        ...
        transit_duration_hours: z.coerce
            .number({ error: t("pages.predict.form.validation.required") })
            .min(0.0, { message: t("pages.predict.form.validation.transit_duration_range") })
            .max(150.0, { message: t("pages.predict.form.validation.transit_duration_range") })
            .refine(
                (val) => !isNaN(val) && val > 0,
                { message: t("pages.predict.form.validation.positive_number") },
            )
        ...
        planet_radius_earth: z.coerce
            .number({ error: t("pages.predict.form.validation.required") })
            .min(0.0, { message: t("pages.predict.form.validation.planet_radius_range") })
            .max(200000.0, { message: t("pages.predict.form.validation.planet_radius_range") })
            .refine(
                (val) => !isNaN(val) && val > 0,
                { message: t("pages.predict.form.validation.positive_number") },
            )
        ...
        equilibrium_temperature_K: z.coerce
            ...
            .max(15000.0, { message: t("pages.predict.form.validation.equilibrium_temp_range") })
            .refine(
                (val) => !isNaN(val) && val > 0,
                { message: t("pages.predict.form.validation.positive_number") },
            )
        ...
        transit_depth_ppm: z.coerce
            ...
            .max(2000000.0, { message: t("pages.predict.form.validation.transit_depth_range") })
            .refine(
                (val) => !isNaN(val) && val > 0,
                { message: t("pages.predict.form.validation.positive_number") },
            )
        ...
        insolation_flux_Earth: z.coerce
            ...
            .max(20000000.0, { message: t("pages.predict.form.validation.insolation_flux_range") })
            .refine(
                (val) => !isNaN(val) && val > 0,
                { message: t("pages.predict.form.validation.positive_number") },
            )
        ...
        stellar_radius_solar: z.coerce
            .number({ error: t("pages.predict.form.validation.required") })
            .min(0.0, { message: t("pages.predict.form.validation.stellar_radius_range") })
            .max(200.0, { message: t("pages.predict.form.validation.stellar_radius_range") })
            .refine(
                (val) => !isNaN(val) && val > 0,
                { message: t("pages.predict.form.validation.positive_number") },
            )
        ...
        stellar_temperature_K: z.coerce
            .number({ error: t("pages.predict.form.validation.required") })
            .min(2000.0, { message: t("pages.predict.form.validation.stellar_temp_range") })
            .max(20000.0, { message: t("pages.predict.form.validation.stellar_temp_range") })
            .refine(
        ...
        signal_to_noise: z.coerce
            .number({ error: t("pages.predict.form.validation.required") })
            .min(0.0, { message: t("pages.predict.form.validation.positive_number") })
            .default(20.0),

        koi_fpflag_nt: z.boolean().default(false),
        koi_fpflag_ss: z.boolean().default(false),
        koi_fpflag_co: z.boolean().default(false),
        koi_fpflag_ec: z.boolean().default(false),
    }),
);
...
const onSubmit = handleSubmit((values) => {
    ...
    // Convert booleans to 0/1 for the API
    const queryParams = {
        ...values,
        koi_fpflag_nt: values.koi_fpflag_nt ? 1 : 0,
        koi_fpflag_ss: values.koi_fpflag_ss ? 1 : 0,
        koi_fpflag_co: values.koi_fpflag_co ? 1 : 0,
        koi_fpflag_ec: values.koi_fpflag_ec ? 1 : 0,
    };
...
<form class="space-y-3 sm:space-y-4" @submit="onSubmit">
    ...
    <div class="space-y-3 sm:space-y-4">
        ...
        <div class="grid grid-cols-1 gap-3 sm:gap-4 items-start">
            ...
            <FormField v-slot="{ componentField }" name="signal_to_noise">
                <FormItem>
                    <FormLabel>
                        {{ $t("pages.predict.form.fields.signal_to_noise.label") }}
                    </FormLabel>
                    <FormControl>
                        <Input
                            :placeholder="
                                $t('pages.predict.form.fields.signal_to_noise.placeholder')
                            "
                            v-bind="componentField"
                        />
                    </FormControl>
                    <FormDescription>
                        {{ $t("pages.predict.form.fields.signal_to_noise.description") }}
                    </FormDescription>
                    <FormMessage />
                </FormItem>
            </FormField>
    ...
    <div class="space-y-3 sm:space-y-4">
        <h3 class="text-sm sm:text-lg font-semibold flex items-center gap-1.5 sm:gap-2">
            🚩 {{ $t("pages.predict.form.sections.flags") }}
        </h3>
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4 items-start">
            <FormField v-slot="{ value, handleChange }" name="koi_fpflag_nt">
                <FormItem class="flex flex-row items-start space-x-3 space-y-0 rounded-md border p-4">
                    ...
                    <div class="space-y-1 leading-none">
                        <FormLabel>
                            {{ $t("pages.predict.form.fields.koi_fpflag_nt.label") }}
                        </FormLabel>
                        <FormDescription>
                            {{ $t("pages.predict.form.fields.koi_fpflag_nt.description") }}
                        </FormDescription>
                    </div>
                </FormItem>
            </FormField>
            <FormField v-slot="{ value, handleChange }" name="koi_fpflag_ss">
                <FormItem class="flex flex-row items-start space-x-3 space-y-0 rounded-md border p-4">
                    <FormControl>
                        <Checkbox :checked="value" @update:checked="handleChange" />
                    </FormControl>
                    <div class="space-y-1 leading-none">
                        <FormLabel>
                            {{ $t("pages.predict.form.fields.koi_fpflag_ss.label") }}
                        </FormLabel>
                        <FormDescription>
                            {{ $t("pages.predict.form.fields.koi_fpflag_ss.description") }}
                        </FormDescription>
                    </div>
                </FormItem>
            </FormField>
            <FormField v-slot="{ value, handleChange }" name="koi_fpflag_co">
                <FormItem class="flex flex-row items-start space-x-3 space-y-0 rounded-md border p-4">
                    <FormControl>
                        <Checkbox :checked="value" @update:checked="handleChange" />
                    </FormControl>
                    <div class="space-y-1 leading-none">
                        <FormLabel>
                            {{ $t("pages.predict.form.fields.koi_fpflag_co.label") }}
                        </FormLabel>
                        <FormDescription>
                            {{ $t("pages.predict.form.fields.koi_fpflag_co.description") }}
                        </FormDescription>
                    </div>
                </FormItem>
            </FormField>
            <FormField v-slot="{ value, handleChange }" name="koi_fpflag_ec">
                <FormItem class="flex flex-row items-start space-x-3 space-y-0 rounded-md border p-4">
                    <FormControl>
                        <Checkbox :checked="value" @update:checked="handleChange" />
                    </FormControl>
                    <div class="space-y-1 leading-none">
                        <FormLabel>
                            {{ $t("pages.predict.form.fields.koi_fpflag_ec.label") }}
                        </FormLabel>
                        <FormDescription>
                            {{ $t("pages.predict.form.fields.koi_fpflag_ec.description") }}
                        </FormDescription>
                    </div>
                </FormItem>
            </FormField>
