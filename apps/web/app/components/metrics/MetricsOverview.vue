...
<template>
    <div class="grid gap-3 sm:gap-4 grid-cols-2 lg:grid-cols-4">
        <template v-for="metric in keyMetrics" :key="metric.label">
            <Card class="p-4 sm:p-6 bg-card border-border hover:border-primary/50 hover:shadow-lg hover:shadow-primary/10 hover:-translate-y-1 transition-all duration-300 cursor-default group">
                <div class="flex items-start justify-between">
                    <div class="space-y-1">
                        <p class="text-xs sm:text-sm font-medium text-muted-foreground group-hover:text-foreground/80 transition-colors">{{ metric.label }}</p>
                    ...
                    <div class="p-1.5 sm:p-2 bg-primary/10 rounded-lg group-hover:bg-primary/20 group-hover:scale-110 transition-all duration-300">
